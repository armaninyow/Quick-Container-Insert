...
import com.armaninyow.quickcontainerinsert.mixin.client.BlockEntityRenderStateAccessor;
...
public class QCIBlockEntityRenderer<T extends BlockEntity>
	...
	public void submit(QCIBlockRenderState renderState, PoseStack poseStack,
		...
		BlockState state = ((BlockEntityRenderStateAccessor) renderState).qci$getBlockState();
		...
		// Provide the real light engine so packed lightmap coords are sampled correctly
		movingState.lightEngine = level.getLightEngine();
		// Standard overworld directional shading — same values vanilla uses in chunk mesh
		movingState.cardinalLighting = CardinalLighting.DEFAULT;
